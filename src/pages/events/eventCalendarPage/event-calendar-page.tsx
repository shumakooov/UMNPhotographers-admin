...
export default function EventCalendarPage() {
    ...
    useEffect(() => {
        const getDataLocation = () => {
            try {
                ...
                axios.get(`https://photographersekb.ru:8080/admin/event/all`, {withCredentials: true})
                    .then(res => {
    ...
    return (
        <div className={styles.wrapper}>
            <CalendarHeader props={{events: events.find((event: Event) => event.id === 1), handleDate: handleDate,}}></CalendarHeader>
            <CalendarGrid props={{
                locations: locations,
                items: activities.filter((i: Activity) => curDate?.isBetween(i.startTime, i.endTime) || curDate?.isSame(i.endTime, 'day')),
                zones: zones,
            }}></CalendarGrid>
        </div>
    );

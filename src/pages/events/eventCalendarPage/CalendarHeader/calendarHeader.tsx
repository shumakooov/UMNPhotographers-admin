...
export default function CalendarHeader({props}: any) {
    let duration = moment.duration(moment(props.events.endTime).diff(moment(props.events.startTime)));

    const [tempDate, setTempDate] = React.useState<Moment>(moment(props.events.startTime));
            ...
            <div className={styles.eventDays}>
                {
                    duration.days() > 0 ? (
                        <>
                            ...
                            <p className={styles.eventDaysText}>День {whichDay}. {tempDate.format('DD.MM.YYYY')}</p>
